 ...
 *
 * 每学期切换当前学期之后，课表不用重导，只需重新物化：
 *
 *   SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/import-course-catalog.mts \
 *     --school uw-madison --materialize-only
 *
 * 为什么要物化：课程流程（202609100002_course_flow）禁止学生建课，学生只能加入
 * 当前学期已经存在的 courses。目录里的课不物化，学生既搜不到也加不了。
 *
...
const { values } = parseArgs({
  options: {
    ...
    "materialize-only": { type: "boolean", default: false },
  },
});

const MATERIALIZE_ONLY = values["materialize-only"] ?? false;

if (!values.school || (!values.file && !MATERIALIZE_ONLY)) {
  console.error(
    [
      "用法：",
      "  node scripts/import-course-catalog.mts --file <xlsx> --school <school_id> [--index <索引页名>] [--apply]",
      "  node scripts/import-course-catalog.mts --school <school_id> --materialize-only",
    ].join("\n"),
  );
  process.exit(1);
}
...
/** 只在真的要写库时才读 service_role key，预演完全不需要它。 */
function serviceClient(): SupabaseClient {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !serviceRoleKey) {
    console.error(
      "\n写库需要环境变量 NEXT_PUBLIC_SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY。\n" +
...
/**
 * 把本校目录物化成当前学期的 courses。可重复执行：已存在的课跳过。
 *
 * 不合规的目录行（课号超过 20 字、课名超过 120 字）会被跳过并计数，而不是截断。
 */
async function materialize(supabase: SupabaseClient): Promise<void> {
  console.log(`\n物化 ${SCHOOL_ID} 的课程目录到当前学期…`);
  const { data, error } = await supabase.rpc("materialize_catalog_courses", {
    target_school: SCHOOL_ID,
  });

  if (error) {
    console.error(`物化失败：${error.message}`);
    console.error(
      "课程目录已经写入，但学生暂时还搜不到。修正后执行 --materialize-only 即可，不用重导课表。",
    );
    process.exit(1);
  }

  const row = (data as MaterializeRow[] | null)?.[0];
  if (!row) {
    console.error("物化没有返回结果，请检查 materialize_catalog_courses 是否已部署。");
    process.exit(1);
  }

  console.log(
    `学期 ${row.materialized_term}：新建 ${row.created_count} 门，已存在 ${row.existing_count} 门`,
  );
  if (row.invalid_count > 0) {
    console.log(
      `另有 ${row.invalid_count} 门因课号超过 20 字或课名超过 120 字未能物化（目录里保留原文，未截断）`,
    );
  }
}

if (MATERIALIZE_ONLY) {
  await materialize(serviceClient());
  process.exit(0);
}
...
console.log(`\n目录写入完成：${SCHOOL_ID} 共 ${written} 门课`);

// ---------------------------------------------------------------------------
// 物化成当前学期可加入的课程
// ---------------------------------------------------------------------------

await materialize(supabase);
