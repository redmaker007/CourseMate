...
`config.toml` 要求用户完成邮箱确认，将验证码固定为 6 位、10 分钟有效，并把同一邮箱再次发送的最短间隔设为 60 秒。新用户使用的 `confirmation` 和已有用户使用的 `magic_link` 模板槽都指向同一份纯 OTP 邮件；这样新用户只有在验证 OTP 后才会触发成员账号绑定。页面倒计时也使用相同的 60 秒规则。
